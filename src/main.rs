...
fn main() {
    ...
    for record in &records {
        ...
        let query = match gandi_query.status() {
            ...
            StatusCode::NOT_FOUND => {
                ...
                match create_record(&api_key, record) {
                    Ok(_) => {
                        println!(
                            "Info: Sucessfull creation for {}.{} type {}",
                            record.get_name(),
                            takina::DOMAIN_NAME,
                            record.get_type()
                        );
                    ...
                    Err(e) => {
                        println!(
                            "API Error: Unable to update for {}.{} type {}",
                            record.get_name(),
                            takina::DOMAIN_NAME,
                            record.get_type()
                        );
        ...
        let res = query
            .text()
            .expect("Parsing Error: Failed to process API response");
        let res: takina::Record =
            serde_json::from_str(&res).expect("Serde Error: Failed to Serialize API response");

        let status = match res.diff(record) {
            takina::RecordState::Diff => update_record(&api_key, record),
            takina::RecordState::Same => {
                println!(
                    "Info: No update needed for {}.{} type {}",
                    record.get_name(),
                    takina::DOMAIN_NAME,
                    record.get_type()
                );
        ...
        match status {
            Ok(_) => {
                println!(
                    "Info: Sucessfull update for {}.{} type {}",
                    record.get_name(),
                    takina::DOMAIN_NAME,
                    record.get_type()
                );
